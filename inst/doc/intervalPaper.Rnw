...
progression-free survival (see e.g., \cite{Frei:2007}), where patients are monitored intermittently 
and progression is known to have occurred only to within the time since the last visit. 

A naïve approach, if one has interval censored data, is to simply
impute the mid-point of the intervals and perform the usual right
censored weighted logrank tests.
\cite{Law:1992} studied this naïve approach
(\cite{Law:1992} use the
term `doubly censored observations' to mean interval censored
observations), and showed through simulation that when the censoring mechanism is different between the two groups, the type I error of a nominal
...
censored data. For each type of rank score (e.g., logrank-type or Wilcoxon-type) the \pkg{interval} package
...
In Section~\ref{sec-background} we give a brief background on the analysis of interval 
censored data, with emphasis on nonparametric maximum likelihood estimation  of the distribution and 
...
of the tests performed by \pkg{perm} are available in the package \pkg{coin} and the exact tests are 
generally faster in \pkg{coin} (\cite{Hoth:2006}). 
...
we detail the design of the \pkg{interval} package. The \pkg{interval} package uses an E-M style algorithm 
...
The default initial estimate uses a function whose main calculation engine uses the \code{computeMLE} function from \pkg{MLEcens}, an available package
 for bivariate interval censored data that is also applicable for univariate interval censored data. Additionally, the \pkg{Icens} package may be used to calculate an initial distribution estimate, and we have  designed the
\pkg{interval} package to be able to input NPMLEs from the \pkg{Icens} package,
using the class structure designed there. Further, we show how the \code{wlr_trafo} function may be used 
seamlessly from within \pkg{coin} to perform exact permutation tests possibly more quickly than using the
default \pkg{perm} package. 

\section{Background on Analysis Methods for Interval Censored
Data}
...
time-to-event variable, but instead of the response time, only whether or not the event has yet occurred 
at a series of assessment times is observed.
...
\subsection{Nonparametric Maximum Likelihood Estimation of the Survival Distribution}
\label{sec-npmle}

With right censored data often we will plot the NPMLE of the survival  
function which in that case is called the Kaplan-Meier or product-limit estimator (see
\cite{Kalb:1980}). The Kaplan-Meier estimator is typically called ``undefined'' after the last observation if that observation is right censored. 
...
of NPMLEs of the survival function by the probability mass associated with the intervals $(0,7]$, $(7,14]$, $\ldots$, $(19,20]$. This class does not 
...
(these intervals are known also as the Turnbull intervals, or the regions of the maximal cliques, see \cite{Gent:2001}).
...
Convergence of the E-M algorithm does not guarantee convergence to the global maximum (see e.g., \cite{Tann:1996}); however, 
...
Kuhn-Tucker conditions are met (see \cite{Gent:1994}). For univariate interval censored data, once the innermost intervals (i.e., regions of the 
maximal cliques) 
are determined, the probability assigned to those intervals which maximizes the likelihood is unique (see e.g., \cite{Gent:2002} Lemma 4).
For bivariate interval censored data, this uniqueness of probabilities may not hold 
and that situation is called {\it mixture non-uniqueness} (see \cite{Gent:2002}).  
...
ratio test) may not follow the usual theory (see \cite{Fay:1996}). Note, however, that the permutation test 
...
methods, or using multiple imputation (\cite{Huan:2008}).
...
unequal censoring the score variance is better (see e.g., \cite{Fay:1996}, p. 820 for the interval censoring case). Further work needs to be done to explore unequal interval censoring; however, we can get some assurance for
the practical use of the permutation method from 
the special case of right censored data, where it has been shown through simulation that the permutation method usually controls the type I error except in extreme cases of unequal censoring and very unbalanced sample sizes between groups (\cite{Hein:2003}). \cite{Hein:2003} also developed an imputation method that controlled the type I simulated error for all cases, and we discuss other related imputation methods applied to interval censored data next. 
...
These methods are provided in the \pkg{survival} package using the \code{survreg} function.
...
uses the marginal likelihood of the ranks (see \cite{Satt:1996}, \cite{Gogg:1999}). The only available software for doing these 
models of which 
we are aware is an S function (which calls a compiled C program requiring access to a SPARC based workstation) to perform a Monte-Carlo EM algorithm for 
proportional hazards models described in \cite{Gogg:1999} available at  
...
(see \cite{Farr:1996} or \cite{Cars:1996}). This is the approach taken with the \code{Icens} function in the \pkg{Epi} package. 
...
three main rank scores available within \pkg{ictest}: those from the logistic (\cite{Sun:1996}), the group proportional hazards (\cite{Fink:1986}), and the generalized Wilcoxon Mann Whitney (\cite{Fay:1996}) models. In the details that follow, we briefly describe the underlying survival model 
(or hazard model) 
...
$S(t_{j} | z_{i}' \beta, \gamma)$. Most of these ways use a model closely related to the 
...
\begin{eqnarray}
\log(X_i) & = & \mu +z_{i}' \beta + \sigma \epsilon_i \label{eq:aft}
\end{eqnarray}
...
\begin{eqnarray}
g(X_i) & = & z_{i}' \beta + \epsilon_i \label{eq:g}
\end{eqnarray}
...
at time $t$ is 
\begin{eqnarray}
S(t |  z_{i}' \beta, \gamma) & = & 1- F\{ g(X_i) - z_{i}' \beta \} \label{eq:Sgcm}
\end{eqnarray}
...
modeled as $S(t_{j} | z_{i}' \beta, \gamma)=S(t_{j} | \gamma)^{\exp(z_i' \beta)}$. This comes from the model of 
...
Next, consider the 
model proposed by \cite{Peto:1972} (see \cite{Fay:1996}) giving the Wilcoxon-type test, 
...
In the following table we summarize the formulation of the scores for the three  model (score) choices in \pkg{interval}, 
...
with radiation therapy with chemotherapy (treatment=``RadChem'') and those treated with radiation therapy alone (treatment=``Rad''). 
...
first visit when the event was observed (right) (or Inf if the event was not observed). 
...
summary(fit1)
@
The \code{Surv} function is from the \pkg{survival} package, and the \code{type="interval2"} treats the variables `left' and `right' as the left and right 
endpoints of the 
interval. The assumption is that the left interval is excluded by the right one is included, except that if both are equal then both are included,
...
These results match those calculated from \pkg{Icens}, an available package for computing the NPMLE for censored and truncated survival data (see \cite{Gent:2001}). The \code{summary} function applied to an \code{icfit} object gives the intervals with 
...
There are five score tests available in \code{ictest}, which are selected by setting the \code{scores} argument to be one of ``logrank1'', ``logrank2'', ``wmw'', 
``normal'',
or ``general''.  As stated in Section~\ref{sec-details}, the two forms of the logrank scores are those associated with \cite{Fink:1986} and those associated with \cite{Sun:1996}. Although \cite{Fink:1986} are perhaps more natural 
...
the NPMLE of the distribution transformed by the inverse of the error distribution from the grouped continuous model (see \cite{Fay:1996}). 
...
which is the default. In Figure~\ref{fig:cll.plot} we plot this using the \code{fit1} icfit object which contains the 
...
if \code{icout} is an ``ictest'' object created by the \code{ictest} function, then \code{icout$scores} will give the vector of rank scores, $c_i$,
...
The imputation method of \cite{Huan:2008} may also be employed (note that \code{scores}=``logrank2'', ``normal'', or ``general'' 
...
NPMLE is on the boundary of the parameter space since some masses where set to zero in the calculation of the NPMLE (although the {\it ad hoc} adjustment 
...
The default method for inference in the \pkg{interval} package is the permutation test. The \pkg{perm} package presented here is a stand-alone R package that performs linear permutation tests. The 
...
and the Wald-type test statistic $Q=(T-U)' V^{-} (T-U)$ is calculated, where $V^{-}$ is the generalized 
inverse of $V$. 
...
are needed. One algorithm supplied is the network algorithm (see e.g., \cite{Agre:1990}). Monte Carlo approximations to the exact 
...
Note that because of the way exact p-values are defined, the way the computer treats ties with $T_0$ and the $T_i$ can lead to non-negligible differences 
...
The data are from an experiment on chicks fed one of four diets. We use only the weight in grams of the chicks at day 21 (i.e., Time=21) 
...
The package uses the S3 methods, which allows object-oriented programming. 
...
For example, as is possible using \code{t.test} or \code{wilcox.test} from the \pkg{stats}, we can call the test using the formula structure, 
...
<<>>=
y3<-ChickWeight[ChickWeight$Time==21 & ChickWeight$Diet==3,"weight"]
y4<-ChickWeight[ChickWeight$Time==21 & ChickWeight$Diet==4,"weight"]
permTS(y3,y4)
@
...
give an quick answer, the default methodRule automatically chooses to use the PCLT. If the sample sizes are small enough, then
exact methods are done automatically. For example, using only the first 5 chicks on each diet, then the methodRule function 
...
by translating portions of the code into \proglang{C} code (see e.g., \cite{Cham:2008}). 

In addition to the \code{pclt} and \code{exact.network} methods, the two-sample test additionally has a complete enumeration 
...
and the Monte Carlo approximation to the exact p-value using the \code{exact.mc} method. The user may supply their own methodRule function, which 
...
output of a methodRule function should be a character vector which is one of ``pclt'', ``exact.network'',
``exact.ce'', or ``exact.mc''. The logical variable \code{exact} causes the default methodRule for \code{permTS} to choose from among the three 
exact algorithms based on the estimated speed of the calculations (see help for \code{methodRuleTS1}). 

All methods except \code{exact.mc} produce an 'htest' object, which is a list with elements described in the help for \code{permTS} and 
printed according to the print method from the \pkg{stats} package.  The  \code{exact.mc} creates an \code{mchtest} object,
 which additionally prints out confidence intervals on the p-value based on the Monte Carlo replications to help the users determine 
 ...
 confidence interval on the p-value is large, the given p-value from the exact.mc method 
 (i.e., [one plus the number of Monte Carlo replications that are equal to or more extreme
 than the observed test statistic] divided by [one plus the number of replications])  
 is always a valid p-value (see e.g., \cite{Fay:2007}, equation 5.3).
...
permKS(y,g)
@
The methodRule function works the same way as for \code{permTS} except a different default methodRule function is used
...
@
Similar methodRule functions may be used (see help for \code{permTREND}).
...
of the options. For example, suppose one wanted to do a simulation on data of the same size as the example. Then one could calculate the 
...
<<>>=
system.time(cm19c10<-chooseMatrix(length(y3)+length(y4),
   length(y3)))
system.time(permTS(y3,y4,method="exact.ce",
   control=permControl(cm=cm19c10)))
...
This section compares the two permutation packages, \pkg{coin} (version 1.0-8) and \pkg{perm} (version 0.9-9.1). 
...
Second, the \pkg{perm} package allows methodRule functions, as previously described, to automatically choose the 
calculation method. No similar feature is offered in \pkg{coin}. Third, when using the Monte Carlo approximation to the 
exact (\code{method="exact.mc"} in \pkg{perm} and distribution=approximate() in \pkg{coin}), then \pkg{perm} prints 
...
test on a small sample size, then the \code{exact.ce} with the cm variable fixed by the control option (so that it does not need to be recalculated 
for each simulation) may give faster results than other algorithms. 
Fifth, the \pkg{perm} package allows two types of two-sided p-values (\code{alternative="two.sided"} and \code{alternative="two.sidedAbs"}), 
while the \pkg{coin} allows only one type of alternative (called \code{alternative="two.sided"} in \pkg{coin} 
but equal to \code{alternative="two.sidedAbs"} in \pkg{perm}). We emphasize that these differences are minor 
...
The \pkg{interval} package uses S3 methods. The two main functions are the \code{icfit} function and the \code{ictest} 
...
as on \code{icfit} object as is possible in the \code{surv} class of the \pkg{survival} package. For example, if the right hand side of the formula 
...
and the `intmap'. The A matrix is an $n \times m$ matrix of zeros and ones with the $ij$th element being an 
 ...
 or $R=\infty$ which is not included. Differences in the inclusion of the endpoints can change the results (see \cite{Ng:2002}).
 ...
 Once an A matrix is calculated and reduced to represent only innermost intervals, the initial estimate of the survival distribution is needed for the E-M algorithm. 
 The \code{initfit} option controls that initial estimate. An allowable option for  \code{initfit} is an object of either class
 \code{icfit} or \code{icsurv}. Another option for  \code{initfit} is a character vector giving the name of a function that inputs five variables 
 (L,R, Lin, Rin, and A; although all need not be used), and outputs a vector of probability masses estimating the distribution and optionally the 
 ...
 Once the initial distribution is calculated then it is used in an E-M algorithm that allows `polishing' elements to zero, 
 then checking the Kuhn-Tucker conditions (see \cite{Gent:1994}). If the initial distribution is very close to the NPMLE, then 
...
will not be met and the \code{message} value of the resulting icfit object  (e.g., \verb@fit$message@) will state this fact.

We test in \code{demo("npmle.R")} that the NPMLE estimates from the \pkg{Icens} package match those from the 
\code{icfit} function. In that file we compare the NPMLE from the two packages for the cosmesis data. We additionally simulate 30 other data
...
There are four choices for a predefined type of rank score: ``logrank1'' (scores described in \cite{Sun:1996}), ``logrank2'' (scores described in \cite{Fink:1986}),
``wmw'' and  ``normal'' (the WMW scores or normal scores described in \cite{Fay:1996}). Additionally, the option ``general'' allows general scores for arbitrary 
error distributions on the grouped continuous model (see \cite{Fay:1996}). To show the general scores we consider the logistic error distribution. We can show 
...
logistic.scores<-ictest(Surv(left,right,type="interval2")~treatment,data=bcos,
    icFIT=icout$fit,scores="general",dqfunc=function(x){ dlogis(qlogis(x))})$scores
...
input must be three objects: the vector of rank scores, the vector of group membership values, and a \code{exact} a logical value coming from 
the object of the 
same name in the input. Other methodRule functions may be created to automatically choose the method based on a function of these three objects, but the 
...
function, \code{wlr_trafo} is an S3 function 
with a default method and one for \code{Surv} objects, but additionally there is a method for \code{data.frame} objects. In the data.frame method, there must be 
only one variable which has either a \code{Surv} or \code{numeric} class. The purpose of the data.frame method is to 
...
@ 
The p-values are different since, as discussed in Section~\ref{sec-perm.coin}, the default two-sided method is different for the \pkg{coin} package than 
the  \pkg{perm} package (and hence also the \pkg{interval} package). When we use \code{alternative="two.sidedAbs"} then we reproduce the results from \pkg{coin}: 
<<>>=
ictest(Surv(left,right,type="interval2")~treatment,data=bcos,subset=SUBSET,method="exact.network",alternative="two.sidedAbs")
@
...
system.time(
ictest(Surv(left,right,type="interval2")~treatment,data=bcos,subset=SUBSET2,method="exact.network",alternative="two.sidedAbs")
)
@
...
example1
@
In this case we can calculate the NPMLE exactly: 
...
We can calculate the exact scores for the Sun method (Eq (\ref{SunScore}); i.e. \code{scores="logrank1"}) these are 
\begin{eqnarray*}
...
Dealing with ties in computer computations can be tricky (see R FAQ 7.31 at \\ \url{http://cran.r-project.org/doc/FAQ/R-FAQ.html}). 
...
p-value is $8/35= 0.2286$. We see that \code{ictest} properly calculates this p-value.
...
We now show how the \code{EMICM} function may be used together with \code{ictest} by employing the \code{initfit} option and compare this to 
